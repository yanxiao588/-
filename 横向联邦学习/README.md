...
> {
	...
	"model_name" : "resnet18",  模型名称，可选用的模型可查看model.py
	
	"no_models" : 10,           客户端数量
	
	"type" : "cifar",           使用的数据集的类型
	
	"global_epochs" : 20,       全局训练轮数，即在所有客户端上进行训练的总轮数。
	
	"local_epochs" : 3,         每个客户端的本地训练轮数，即每个客户端在每一轮全局训练需要进行的训练轮数。
	
	"k" : 5,                    每轮全局训练中随机选择的客户端数量。
	
	"batch_size" : 32,          每个批次的样本数量，用于在训练过程中更新模型。
	
	"lr" : 0.001,               学习率，用于控制模型在训练过程中的学习速度。
	
	"momentum" : 0.0001,        这是动量，用于加速模型在训练过程中的收敛速度。
	
	"lambda" : 0.1              正则化参数，用于防止模型过拟合。
}
以上参数可根据需要更改
...
- 定义构造函数。在构造函数中，服务端的工作包括：第一，将配置信息拷贝到服务端中；第二，按照配置中的模型信息获取模型，这里使用torchvision 的models模块内置的ResNet-18模型。
...
- 定义模型聚合函数。前面已经提到服务端的主要功能是进行模型的聚合，因此定义构造函数后，需要在类中定义模型聚合函数，通过接收客户端上传的模型，使用聚合函数更新全局模型。聚合方案有很多种，此处采用经典的FedAvg 算法。
...
横向联邦学习的客户端主要功能是接收服务端的下发指令和全局模型，利用本地数据进行局部模型训练。首先定义客户端类Client，类中的主要函数包括以下两种。
...
当配置文件、服务端类和客户端类都定义完毕，将这些信息组合起来。首先，读取配置文件信息。
```python
with open(args.conf, 'r') as f:
	conf = json.load(f)	
```
接下来，将分别定义一个服务端对象和多个客户端对象，用来模拟横向联邦训练场景。
```python
